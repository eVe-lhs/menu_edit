...
export function showCategoryForm(category = null) {
  if (category) {
    ...
    document.getElementById("categoryVnName").value = category.vietnamese_name || "";
    ...
    document.getElementById("categoryVnName").value = "";
...
export async function showItemForm(item = null) {
  ...
  // Regular item mode
  document.getElementById("itemFormTitle").textContent = item ? "Edit Item" : "Add New Item";
  ...
  document.getElementById("itemVnNameField").style.display = "block";
  ...
  if (item) {
    ...
    document.getElementById("itemVnName").value = item.VI_Name || "";
    document.getElementById("itemCategory").value = item.Category;
    document.getElementById("itemFoodStatus").value = item.Food_Status;
    document.getElementById("itemRestaurantSection").value = item.Restaurant_Section;
    ...
    document.getElementById("itemVnName").value = "";
    document.getElementById("itemCategory").value = appState.categories.length > 0 ? appState.categories[0].name : "";
...
export function showOptionForm(item = null) {
  ...
  document.getElementById("itemFormTitle").textContent = item ? "Edit Option" : "Add New Option";
  ...
  document.getElementById("itemVnNameField").style.display = "none";
  ...
  const sortedBaseItems = [...baseItems].sort((a, b) => a.Code.localeCompare(b.Code));
  ...
  codeSelect.onchange = () => {
    ...
    if (selectedItem) {
      ...
      document.getElementById("baseItemVnName").value = selectedItem.VI_Name || "";
  ...
  if (item) {
    ...
    const vnMatch = item.VI_Name ? item.VI_Name.match(/\[(.*?)\]/) : null;

    document.getElementById("itemNameOption").value = nameMatch ? nameMatch[1] : "";
    document.getElementById("itemJpNameOption").value = jpMatch ? jpMatch[1] : "";
    document.getElementById("itemMmNameOption").value = mmMatch ? mmMatch[1] : "";
    document.getElementById("itemCnNameOption").value = cnMatch ? cnMatch[1] : "";
    document.getElementById("itemVnNameOption").value = vnMatch ? vnMatch[1] : "";
    currentItemId = appState.items.indexOf(item);
  } else {
    // reset
    codeSelect.value = "";
    ...
    document.getElementById("itemVnNameOption").value = "";
    currentItemId = null;
  }
...
export function saveCategory() {
  ...
  const vnName = document.getElementById("categoryVnName").value.trim();
  ...
  const category = {
    ...
    myanmar_name: mmName,
    chinese_name: cnName,
    vietnamese_name: vnName || "", // Default to empty if not provided
  };
...
export function deleteCategory(index) {
  if (confirm("Are you sure you want to delete this category?")) {
    ...
    const itemsUsingCategory = appState.items.filter((item) => item.Category === categoryName);
    if (itemsUsingCategory.length > 0) {
      alert("Cannot delete this category because it is being used by one or more items.");
      return;
    }
...
export function saveItem() {
  const isOptionMode = document.getElementById("optionFields").style.display === "block";
  let code, name, jpName, mmName, cnName, vnName, category, image;

  if (isOptionMode) {
    code = document.getElementById("itemCodeSelect").value;
    const baseItem = appState.items.find((i) => i.Code === code && !i.Name.includes("["));
    if (!baseItem) {
    ...
    const jpNameOption = document.getElementById("itemJpNameOption").value.trim();
    const mmNameOption = document.getElementById("itemMmNameOption").value.trim();
    const cnNameOption = document.getElementById("itemCnNameOption").value.trim();
    const vnNameOption = document.getElementById("itemVnNameOption").value.trim();
    ...
    vnName = `${baseItem.VI_Name || ""} [${vnNameOption || nameOption}]`;
    ...
    vnName = document.getElementById("itemVnName").value.trim();
    ...
    if (imageFile) {
      ...
      const newFileName = `${code}_${timestamp}${fileName.substring(fileName.lastIndexOf("."))}`;
      image = newFileName;
  ...
  const restaurantSection = document.getElementById("itemRestaurantSection").value;
  ...
  const priceWithTax = (parseFloat(price) * (1 + parseFloat(tax) / 100)).toFixed(0);
...
export function deleteItem(index) {
  const item = appState.items[index];
  const hasOptions = appState.items.some((i) => i.Code === item.Code && i !== item && i.Name.includes("["));

  if (hasOptions) {
    alert("Cannot delete this item because it has options. Please delete the options first.");
    return;
  }
...
export function saveTable() {
  ...
  const table = {
    id: currentTableId !== null ? appState.tables[currentTableId].id : appState.tables.length + 1,
    number,
    type,
    restaurant,
  };
