...
export function renderCategoriesTable() {
  ...
  appState.categories.forEach((category, index) => {
    ...
    row.innerHTML = `
            ...
            <td>${category.vietnamese_name}</td>
            <td>
  ...
  document.querySelectorAll(".edit-category").forEach((button) => {
    button.addEventListener("click", (e) => editCategory(e.target.dataset.index));
  });
  document.querySelectorAll(".delete-category").forEach((button) => {
    button.addEventListener("click", (e) => deleteCategory(e.target.dataset.index));
  });
}
...
export function updateCategoryDropdown() {
  ...
  if (currentValue && appState.categories.some((cat) => cat.name === currentValue)) {
