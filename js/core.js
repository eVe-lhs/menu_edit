...
export async function importFromZip(event) {
  ...
  if (!file) return;
  console.log("Importing from zip file:", file.name);

  try {
    const zip = await JSZip.loadAsync(file);
    let targetFolder = null;
    zip.forEach((relativePath, zipEntry) => {
      if (!zipEntry.dir && (relativePath.endsWith("categories.csv") || relativePath.endsWith("items.csv"))) {
    ...
    if (categoriesCsvFile) {
      ...
      lines.forEach((line) => {
        if (line.trim()) {
          const [name, japan_name, myanmar_name, chinese_name, vietnamese_name] = parseCSVLine(line);
          if (name && japan_name) {
            newCategories.push({
              name: name.trim(),
              ...
              vietnamese_name: vietnamese_name ? vietnamese_name.trim() : "", // Default to empty, will be updated later
            });
          }
        }
      });
    ...
    if (itemsCsvFile) {
      ...
      console.log("Line count in items.csv:", lines.length);
      lines.forEach((line) => {
        console.log("Processing line:", line, "Line number:", lines.indexOf(line) + 1);
        if (line.trim()) {
          const fields = parseCSVLine(line);
          const [Code, Name, JP_Name, MM_Name, CN_Name, VI_Name, Category, Food_Status, Restaurant_Section, Price, Tax, Price_Included_Tax, Image] = fields;
          if (Code && Name) {
            newItems.push({
              Code: Code.trim(),
              Name: Name.trim(),
              ...
              VI_Name: VI_Name ? VI_Name.trim() : "", // Default to empty, will be updated later
              Category: Category ? Category.trim() : "",
              Food_Status: Food_Status ? Food_Status.trim() : "Available",
              Restaurant_Section: Restaurant_Section ? Restaurant_Section.trim() : "NongInlay",
              Price: Price ? Price.trim() : "0",
              Tax: Tax ? Tax.trim() : "0",
              Price_Included_Tax: Price_Included_Tax ? Price_Included_Tax.trim() : "0",
        ...
        if (line.trim()) {
          ...
          if (id && number) {
            newTables.push({
              ...
              restaurant: restaurant ? restaurant.trim() : "Default",
            });
          }
        }
      });
...
export function exportToZip() {
  if (appState.categories.length === 0 || appState.items.length === 0 || appState.tables.length === 0) {
    alert("Please add at least one category, one table and one item before exporting");
    return;
  }

  const zip = new JSZip();
  const today = new Date();
  const folderName = `menu_data_${today.getFullYear()}${(today.getMonth() + 1).toString().padStart(2, "0")}${today.getDate().toString().padStart(2, "0")}${today.getHours().toString().padStart(2, "0")}${today
    .getMinutes()
    .toString()
  ...
  let categoriesCsv = "name,japan_name,myanmar_name,chinese_name,vietnamese_name\n";
  appState.categories.forEach((cat) => {
    categoriesCsv += `${escapeCSVField(cat.name)},${escapeCSVField(cat.japan_name)},${escapeCSVField(cat.myanmar_name)},${escapeCSVField(cat.chinese_name)},${escapeCSVField(cat.vietnamese_name)}\n`;
  });

  let itemsCsv = "Code,Name,JP Name,MM Name,CN Name,VI Name,Category,Food Status,Restaurant Section,Price,Tax,Price Included Tax,Image\n";
  appState.items.forEach((item) => {
    itemsCsv += `${escapeCSVField(item.Code)},${escapeCSVField(item.Name)},${escapeCSVField(item.JP_Name)},${escapeCSVField(item.MM_Name)},${escapeCSVField(item.CN_Name)},${escapeCSVField(item.VI_Name)},${escapeCSVField(
      item.Category
    )},${escapeCSVField(item.Food_Status)},${escapeCSVField(item.Restaurant_Section)},${escapeCSVField(item.Price)},${escapeCSVField(item.Tax)},${escapeCSVField(item.Price_Included_Tax)},${escapeCSVField(item.Image)}\n`;
  });
